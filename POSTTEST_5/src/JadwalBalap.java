...
public class JadwalBalap implements IKelola {
    private String namaGrandPrix;
    private String lokasi;
    private String tanggal;
    protected int putaran;

    private static ArrayList<JadwalBalap> listJadwal;
    ...
    public JadwalBalap(ArrayList<JadwalBalap> list) {
        listJadwal = list;
    }

    public String getNamaGrandPrix() { return namaGrandPrix; }
    public String getLokasi() { return lokasi; }
    public String getTanggal() { return tanggal; }
    public int getPutaran() { return putaran; }
    ...
    public static void urutkan(ArrayList<JadwalBalap> list) {
        int n = list.size();
        for (int i = 0; i < n - 1; i++) {
            for (int j = 0; j < n - i - 1; j++) {
                if (list.get(j).getPutaran() > list.get(j + 1).getPutaran()) {
                    JadwalBalap temp = list.get(j);
    ...
    @Override
    public void ubah(Scanner scanner) {
        lihatSemua();
    ...
    public static void tambah(ArrayList<JadwalBalap> list, Scanner scanner) {
        JadwalBalap manager = new JadwalBalap(list);
        manager.tambah(scanner);
    }

    public static void lihatSemua(ArrayList<JadwalBalap> list) {
        JadwalBalap manager = new JadwalBalap(list);
        manager.lihatSemua();
    }

    public static void ubah(ArrayList<JadwalBalap> list, Scanner scanner) {
        JadwalBalap manager = new JadwalBalap(list);
        manager.ubah(scanner);
    }

    public static void hapus(ArrayList<JadwalBalap> list, Scanner scanner) {
        JadwalBalap manager = new JadwalBalap(list);
        manager.hapus(scanner);
    }
}